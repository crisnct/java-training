 ...
 *
 *  Java 15:
 *  - ZGC GC is not experimental anymore so can be used just like this: -XX:+UseZGC -Xlog:gc
 *  - Shenandoah GC is not experimental anymore so can be used just like this: -XX:+UseShenandoahGC -Xlog:gc
 *
 *  Java 18:
 *    GC and runtime tweaks
 *    String deduplication support extended to more garbage collectors (Serial, Parallel, ZGC) in 18, not just G1/Shenandoah.
 *    G1 heap region size allowed up to 512 MB, which helps very large heaps reduce metadata overhead.
 */
// @formatter:on
